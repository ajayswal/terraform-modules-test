...
/**
 * As improvement, we might want to allow people to pass in the state name
 * For now, we generate file name statically by appending `terraform.tfstate`
 * to the path they have provided in URL
 */

function makeFilename(env) {
  if (env) {
    return `${env}/terraform.tfstate`;
  } else {
...
function makeVersionFilename(env, serial) {
  if (env) {
    return `${env}/terraform-${serial}.tfstate`;
  } else {
    return `terraform-${serial}.tfstate`;
  }
}

function makeLockFilename(env) {
  if (env) {
    return `${env}/terraform.lock`;
  } else {
...
async function main(params) {
  ...
  // Parse extra path and get bucket name as first element in path
  const bucket = params.__ow_path.split('/')[2];
  const env = params.__ow_path.split('/').slice(3).join('/').toString();
  const queryParams = params.__ow_query ? querystring.parse(params.__ow_query) : {};
  const versioning = params['versioning'];
  ...
  const state = new State(storage, env, versioning);
